...
INSERT INTO public.achievements (name, description, category, points_value, requirements) VALUES
  ...
  ('Teacher''s Pet', 'Submit 10 assignments on time', 'dedication', 250, '{"type": "assignment_submission", "on_time_count": 10}');

-- Insert sample forums
INSERT INTO public.forums (title, description, category, created_by) VALUES
  ('General Discussion', 'Talk about anything related to learning', 'general', (SELECT user_id FROM profiles WHERE user_type = 'teacher' LIMIT 1)),
  ('Mathematics Help', 'Get help with math problems and concepts', 'subject', (SELECT user_id FROM profiles WHERE user_type = 'teacher' LIMIT 1)),
  ('Science Corner', 'Discuss science topics and experiments', 'subject', (SELECT user_id FROM profiles WHERE user_type = 'teacher' LIMIT 1)),
  ('Study Tips', 'Share and discover effective study techniques', 'tips', (SELECT user_id FROM profiles WHERE user_type = 'teacher' LIMIT 1));

-- Insert sample resources
INSERT INTO public.resources (title, description, resource_type, content_url, subject, grade_level, tags, created_by) VALUES
  ('Math Formula Sheet', 'Essential formulas for algebra and geometry', 'pdf', 'https://example.com/math-formulas.pdf', 'Mathematics', 'Grade 8-10', ARRAY['formulas', 'reference', 'algebra', 'geometry'], (SELECT user_id FROM profiles WHERE user_type = 'teacher' LIMIT 1)),
  ('Science Experiment Videos', 'Safe home experiments for kids', 'video', 'https://youtube.com/playlist', 'Science', 'Grade 6-8', ARRAY['experiments', 'videos', 'hands-on'], (SELECT user_id FROM profiles WHERE user_type = 'teacher' LIMIT 1)),
  ('Study Planner Template', 'Organize your study schedule effectively', 'document', 'https://example.com/study-planner.doc', 'Study Skills', 'All Grades', ARRAY['planning', 'organization', 'template'], (SELECT user_id FROM profiles WHERE user_type = 'teacher' LIMIT 1));